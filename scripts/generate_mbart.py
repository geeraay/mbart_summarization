...
parser.add_argument('--lang-id', type=str, default=None, help='language id')
...
try:
    bart = BARTModel.from_pretrained(
        ...
        # data_name_or_path='data-bin/{}/mbart/'.format(args.dataset),
        data_name_or_path='data-bin/{}/mbart/{}'.format(args.dataset, args.src),
        layernorm_embedding=True,
        bpe='sentencepiece',
        sentencepiece_vocab='./mbart50.pretrained/sentence.bpe.model',
...
if args.rc:
     bsz = 1
with open('datasets/{}/{}'.format(args.dataset, args.src), encoding='utf8') as source, open(os.path.join(args.exp, '{}.{}.hypo'.format(args.src, args.outfix)), 'w') as fout:
    ...
    for sline in source:
        if count % bsz == 0:
            ...
            for i, hypothesis in enumerate(hypotheses_batch):
                ...
                if hypothesis.startswith(f"[{args.lang_id}]"):
                    hypothesis = hypothesis.split(f"[{args.lang_id}]")[1]
